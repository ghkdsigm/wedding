<template>
  ...
  <section class="relative max-w-[640px] mx-auto w-full bg-white text-[#3f3a37] overflow-hidden pt-12">
    ...
    <div class="relative mx-auto w-full max-w-md px-5 py-5 space-y-4">
      ...
      <div class="rounded-xl border border-[#eee] bg-white/90 backdrop-blur p-4">
        <h3 class="text-base font-semibold break-words">{{ venueName }}</h3>
        <p v-if="address" class="text-sm text-[#6c625b] mt-0.5 break-words whitespace-pre-line">{{ address }}</p>
        <p v-if="tel" class="text-[clamp(10px,3vw,14px)] text-[#6c625b] mt-0.5 whitespace-pre-line">Tel. {{ tel }}</p>
      </div>

      <!-- Action buttons -->
      <div v-if="tel || address" class="grid grid-cols-2 gap-2 pb-12">
        <button v-if="tel" @click="callTel" class="rounded-full bg-white px-3 py-2 border border-[#ddd] text-sm hover:bg-[#5a4a3d] active:scale-95 transition-all duration-150">전화하기</button>
        <button v-if="address" @click="copyAddress" class="rounded-full text-white bg-[#6b5a4d] px-3 py-2 text-sm hover:bg-gray-50 active:scale-95 transition-all duration-150">주소 복사</button>
      </div>
      ...
      <div class="pt-4 space-y-5">
        <div>
          <h4 class="text-base font-bold">지하철안내</h4>
          <p class="text-sm text-[#6c625b] mt-1 leading-6">{{ subwayInfo }}</p>
        </div>
        <div class="h-px bg-[#eee]"></div>
        <div>
          <h4 class="text-base font-bold">버스안내</h4>
          <p class="text-sm text-[#6c625b] mt-1 leading-6 whitespace-pre-line">{{ busInfo }}</p>
        </div>
        <div class="h-px bg-[#eee]"></div>
        <div>
          <h4 class="text-base font-bold">주차안내</h4>
          <p class="text-sm text-[#6c625b] mt-1 leading-6">{{ parkingInfo }}</p>
        </div>
      </div>
    </div>
  </section>
